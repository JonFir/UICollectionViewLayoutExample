...
func makeCustomLayoutVC6StateMachine() -> UIViewController {
    CustomLayoutVC()
}

private class CustomLayoutVC: UIViewController {

    var texts = (0..<100).map { _ in makeTexts(lenght: 100) }
...
private class CustomLayout: UICollectionViewLayout {
    ...
    private var needRebuildLayout = false
    ...
    override func prepare() {
        ...
        guard let collectionView, let delegate, needRebuildLayout else { return }

        let contentWidth = collectionView.frame.width
        let sections = Self.makeSections(view: collectionView, delegate: delegate, contentWidth: contentWidth)
...
private class CustomLayoutState {
    ...
    private func rebuildSectionFrame(sectionIndex: Int, startAtItemIndex startItemIndex: Int, contentWidth: Double) {
        ...
        for itemIndex in startItemIndex..<currentSections[sectionIndex].items.count {
            ...
            if indexInRow == sectionConfiguration.numberOfCollumn - 1 {
